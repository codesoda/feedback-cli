...
- History archiving lives in `src/history.rs`; Done writes the transcript payload JSON to `<history_dir>/<sanitized-source-stem>/<ISO8601>.json` after stdout emission, and archive failures must warn to tracing/stderr without changing the `/api/done` response or shutdown.
